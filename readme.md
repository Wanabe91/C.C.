...
Candidate facts reviewed for memory tiering
  -> consolidator proposes merge and/or tier_change operations
  -> next ingest_event() applies pending proposals
  -> merge sources moved to cold tier
  -> merged fact inserted and queued for embedding
  -> archived status only set after an explicit archived tier_change
```
...
As a side effect, any fact surfaced through FTS, vector search, or `delta_facts` has its
`last_accessed_at` and `access_count` updated in SQLite.
...
Before planning, `apply_pending_proposals(V)` checks whether the consolidator has already proposed memory-tiering operations.

For each pending proposal:

1. Parse the normalized proposal payload.
2. Re-load referenced facts in a transaction.
3. Reject the proposal if referenced facts are missing, inactive, or inconsistent with proposal rules.
4. For a `merge`, insert one merged fact with metadata including `merged_from`, queue it for embedding, and set source facts to `tier = 'cold'`.
5. For a `tier_change` to `cold`, update that fact's tier only.
...
3. Parses plan JSON into `PlanStep` objects inside a `PlannerRun` record.
...
- `remember_fact`
  Produces new fact payloads to persist, including optional `importance` / `tier`.
- `create_task`
  Produces task rows to persist.
- `complete_task`
  Produces task IDs to mark completed.
- `generate_weekly_review`
  Produces a weekly review artifact and assistant-visible Markdown.
- `noop`
...
- the processed event
- decision facts (`meta.kind = "decision"`)
...
- `events`
  Raw ingested events.
- `facts`
  Durable memory units with `status`, `importance`, `tier`, and access metadata.
- `tasks`
...
- `planner_runs`
  Captured planner inputs, raw outputs, repaired outputs, and final step lists.
- `step_traces`
  Per-step revalidation and execution trace rows.
- `weekly_reviews`
...
This snapshot is passed as JSON to the planner LLM, and any retrieved fact is marked as accessed for future tiering decisions.
...
`memory_engine/consolidator.py` periodically asks the LLM for memory-tiering proposals over a mixed pool of recent and stale active facts.

Flow:

1. Load a candidate set that includes both recent active facts and stale active facts.
2. Serialize each fact with `id`, `content`, `importance`, `tier`, `created_at`, `last_accessed_at`, and `access_count`.
3. Ask the LLM to return a strict JSON object containing up to five `merge` and/or `tier_change` proposals.
4. Normalize and validate those proposals locally.
5. Skip proposals already pending.
6. Insert the remaining proposals for later application by `ingest_event()`.

Important design detail:
The consolidator does not apply tiering changes directly. It only proposes them. Actual mutation happens synchronously inside the next `ingest_event()` call.
...
- `consolidator.py`
  Async memory-tiering proposal worker.
- `embeddings.py`
...
- `weekly_review.py`
  Builds weekly review summaries and Markdown drafts from persisted activity.
- `working_memory.py`
...
5. Fact consolidation and tiering are proposal-based.
   Background code suggests `merge` / `tier_change` operations, but foreground code applies them.
...
`main.py` feeds user text into `memory_engine.loop.ingest_event()`, which persists the event, builds mixed SQLite/Chroma context, updates fact access metadata, asks the LLM for structured plan steps, executes those steps into durable state changes, writes review and note artifacts, and relies on background workers to maintain embeddings and proposal-based memory tiering over time.
